...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  // NOTE: I removed the code to identify min distance landmarks from here and integrated in directly into updateWeights because it saved a loop through all the observations, which should improve performance.
}
  ...
  for (int p = 0; p < num_particles; p++) {
    ...
    for (int i = 0; i < trans_observations.size(); i++) {
      ...
      for (int j = 0; j < map_landmarks.landmark_list.size(); j++) {
        ...
        double dist = sqrt(pow(trans_observations[i].x - landmark_x, 2.0) + pow(trans_observations[i].y - landmark_y, 2.0));
        if (dist < min_dist) {
          min_dist = dist;
          association = j;
        }
